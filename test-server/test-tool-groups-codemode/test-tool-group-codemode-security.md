# postgres-mcp codemode Re-Testing: [security]
...
1. `pg.security.audit()` → verify `{success: true, findings: [...], summary: {total, passed, warnings, critical}}`
2. `pg.security.audit({limit: 2})` → verify findings array length ≤ 2
3. `pg.security.sslStatus()` → verify `{success: true, sslEnabled: boolean}`
4. `pg.security.encryptionStatus()` → verify `{success: true, sslEnabled, passwordEncryption, pgcryptoAvailable}`
5. `pg.security.passwordValidate({password: "Str0ng!Pass#2026"})` → verify `strength >= 80`, `interpretation: "Very Strong"`
6. `pg.security.passwordValidate({password: "123456"})` → verify `strength < 20`, `checks.notCommon: false`
7. `pg.security.maskData({value: "user@example.com", type: "email"})` → verify masked output preserves domain, local part masked
8. `pg.security.maskData({value: "4111111111111111", type: "credit_card"})` → verify first/last 4 preserved
9. `pg.security.maskData({value: "555-12-3456", type: "ssn"})` → verify last 4 digits preserved
10. `pg.security.maskData({value: "sensitive-data", type: "partial", keepFirst: 3, keepLast: 2})` → verify `sen*********ta`
11. `pg.security.userPrivileges()` → verify `{success: true, users: [...], count: N}`
12. `pg.security.userPrivileges({user: "postgres"})` → verify single role with `isSuperuser: true`
13. `pg.security.userPrivileges({summary: true})` → verify condensed output with `grantCount`
14. `pg.security.sensitiveTables()` → verify returns matches with default patterns
15. `pg.security.sensitiveTables({schema: "public", patterns: ["email", "password"]})` → verify custom patterns
16. `pg.security.firewallStatus()` → verify `{success: true}` with HBA data or structured error
17. `pg.security.firewallRules()` → verify `{success: true, rules: [...]}` or structured error

18. 🔴 `pg.security.passwordValidate({})` → `{success: false, error: "..."}` (missing password)
19. 🔴 `pg.security.maskData({})` → `{success: false, error: "..."}` (missing value and type)
20. 🔴 `pg.security.maskData({value: "test", type: "invalid_type"})` → `{success: false, error: "..."}` handler error
21. 🔴 `pg.security.userPrivileges({user: "nonexistent_role_xyz"})` → `{success: false, error: "Role 'nonexistent_role_xyz' does not exist."}`
22. 🔴 `pg.security.sensitiveTables({schema: "fake_schema_xyz"})` → `{success: false, error: "Schema 'fake_schema_xyz' does not exist..."}`
23. 🔴 `pg.security.firewallRules({type: 999})` → `{success: false, error: "..."}` (Zod type mismatch)
