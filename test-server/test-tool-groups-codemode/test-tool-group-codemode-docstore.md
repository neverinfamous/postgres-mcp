# postgres-mcp codemode Re-Testing: [docstore]
...
- **Temporary collections**: Prefix with `temp_` (e.g., `temp_doc_cm_test`)
...
**Test data:** Docstore tools operate on JSONB document collections — tables with a `_id` TEXT primary key and `doc` JSONB column. The `test_documents` collection provides 5 seed documents with `name`, `age`, `tags` (array), and `address` (nested object) fields. Create `temp_doc_cm_test` for write operation testing via `pg.docstore.createCollection`.

**Checklist:**

1. `pg.docstore.listCollections()` → verify `test_documents` present in results
2. `pg.docstore.listCollections({schema: "public"})` → verify schema filter works
3. `pg.docstore.collectionInfo({collection: "test_documents"})` → verify `{success: true}` with rowCount (5)
4. `pg.docstore.find({collection: "test_documents"})` → verify returns 5 docs
5. `pg.docstore.find({collection: "test_documents", filter: {name: "Alice"}})` → verify returns 1 doc
6. `pg.docstore.find({collection: "test_documents", filter: {age: {$gt: 30}}})` → verify returns 2 docs (Charlie, Eve)
7. `pg.docstore.find({collection: "test_documents", limit: 2})` → verify returns exactly 2 docs
8. `pg.docstore.find({collection: "test_documents", fields: ["name", "age"]})` → verify projected fields only
9. `pg.docstore.createCollection({collection: "temp_doc_cm_test"})` → verify created
10. `pg.docstore.add({collection: "temp_doc_cm_test", documents: [{name: "CM1", value: 100}, {name: "CM2", value: 200}]})` → verify 2 added
11. `pg.docstore.find({collection: "temp_doc_cm_test"})` → verify returns 2 docs with auto-generated `_id`
12. `pg.docstore.modify({collection: "temp_doc_cm_test", filter: {name: "CM1"}, set: {status: "done"}})` → verify modified
13. `pg.docstore.find({collection: "temp_doc_cm_test", filter: {status: "done"}})` → verify 1 doc with `status: "done"`
14. `pg.docstore.remove({collection: "temp_doc_cm_test", filter: {status: "done"}})` → verify 1 removed
15. `pg.docstore.find({collection: "temp_doc_cm_test"})` → verify 1 doc remaining
16. `pg.docstore.createIndex({collection: "temp_doc_cm_test", field: "name"})` → verify index created
17. `pg.docstore.dropCollection({collection: "temp_doc_cm_test"})` → verify dropped

18. 🔴 `pg.docstore.find({})` → `{success: false, error: "..."}` (missing collection)
19. 🔴 `pg.docstore.add({})` → `{success: false, error: "..."}` (missing params)
20. 🔴 `pg.docstore.find({collection: "nonexistent_collection_xyz"})` → `{success: false, error: "..."}` (P154)
21. 🔴 `pg.docstore.collectionInfo({collection: "nonexistent_collection_xyz"})` → `{success: false, error: "..."}` (P154)
22. 🔴 `pg.docstore.modify({collection: "nonexistent_collection_xyz", filter: {}, set: {x: 1}})` → `{success: false, error: "..."}` (P154)
23. 🔴 `pg.docstore.remove({collection: "nonexistent_collection_xyz", filter: {}})` → `{success: false, error: "..."}` (P154)
24. 🔴 `pg.docstore.createCollection({collection: "test_documents"})` → `{success: false, error: "..."}` (duplicate)
25. 🔴 `pg.docstore.createIndex({})` → `{success: false, error: "..."}` (missing params)
26. 🔴 `pg.docstore.listCollections({schema: "fake_schema_xyz"})` → `{success: false, error: "..."}` (P154)
