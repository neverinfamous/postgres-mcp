# postgres-mcp codemode Re-Testing: [cron]
...
13. [x] `pg_cron_alter_job()` → verify happy path expected behavior
14. [x] 🔴 `pg_cron_alter_job({})` → verify structured P154 error response or valid defaults
15. [x] `pg_cron_job_run_details()` → verify happy path expected behavior
16. [x] 🔴 `pg_cron_job_run_details({})` → verify structured P154 error response or valid defaults
17. [x] `pg_cron_create_extension()` → verify happy path expected behavior
18. [x] 🔴 `pg_cron_create_extension({})` → verify structured P154 error response or valid defaults
19. [x] `pg_cron_schedule_in_database()` → verify happy path expected behavior
20. [x] 🔴 `pg_cron_schedule_in_database({})` → verify structured P154 error response or valid defaults
