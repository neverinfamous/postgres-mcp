...
| 2 | `postgres://tables` | `{ tables: [...] }` | Returns 10+ tables (test_* tables from test-database.sql) with sizes |
...
| 11 | `postgres://indexes` | `{ totalIndexes, indexDetails: [...] }` | Returns index info for test tables (idx_orders_*, idx_articles_fts, etc.) |
