# postgres-mcp Tool Group Re-Testing: [monitoring]
...
12. `pg_replication_status()` → verify happy path expected behavior
13. 🔴 `pg_replication_status({})` → verify structured P154 error response or valid defaults
14. `pg_capacity_planning()` → verify happy path expected behavior
15. 🔴 `pg_capacity_planning({})` → verify structured P154 error response or valid defaults
16. `pg_resource_usage_analyze()` → verify happy path expected behavior
17. 🔴 `pg_resource_usage_analyze({})` → verify structured P154 error response or valid defaults
