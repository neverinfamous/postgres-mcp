# postgres-mcp Tool Group Re-Testing: [partitioning]
...
6. `pg_attach_partition()` → verify happy path expected behavior
7. 🔴 `pg_attach_partition({})` → verify structured P154 error response or valid defaults
8. `pg_detach_partition()` → verify happy path expected behavior
9. 🔴 `pg_detach_partition({})` → verify structured P154 error response or valid defaults
10. `pg_create_partitioned_table()` → verify happy path expected behavior
11. 🔴 `pg_create_partitioned_table({})` → verify structured P154 error response or valid defaults
12. `pg_create_partition()` → verify happy path expected behavior
13. 🔴 `pg_create_partition({})` → verify structured P154 error response or valid defaults
