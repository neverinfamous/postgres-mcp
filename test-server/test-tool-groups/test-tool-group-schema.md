# postgres-mcp Tool Group Re-Testing: [schema]
...
9. `pg_create_schema()` → verify happy path expected behavior
10. 🔴 `pg_create_schema({})` → verify structured P154 error response or valid defaults
11. `pg_drop_schema()` → verify happy path expected behavior
12. 🔴 `pg_drop_schema({})` → verify structured P154 error response or valid defaults
13. `pg_drop_sequence()` → verify happy path expected behavior
14. 🔴 `pg_drop_sequence({})` → verify structured P154 error response or valid defaults
15. `pg_create_view()` → verify happy path expected behavior
16. 🔴 `pg_create_view({})` → verify structured P154 error response or valid defaults
17. `pg_drop_view()` → verify happy path expected behavior
18. 🔴 `pg_drop_view({})` → verify structured P154 error response or valid defaults
