...
**Directory Purpose**: This folder contains 29 self-contained, modular test prompts covering every tool group in `postgres-mcp`. Unlike the `test-tool-groups-codemode.md` directory, these prompts are strictly designed for **Direct MCP Tool Call validation**.
