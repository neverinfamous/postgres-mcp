# postgres-mcp Tool Group Re-Testing: [kcache]
...
6. `pg_kcache_create_extension()` → verify happy path expected behavior
7. 🔴 `pg_kcache_create_extension({})` → verify structured P154 error response or valid defaults
8. `pg_kcache_resource_analysis()` → verify happy path expected behavior
9. 🔴 `pg_kcache_resource_analysis({})` → verify structured P154 error response or valid defaults
10. `pg_kcache_reset()` → verify happy path expected behavior
11. 🔴 `pg_kcache_reset({})` → verify structured P154 error response or valid defaults
