# postgres-mcp Tool Group Re-Testing: [security]
...
**Test data:** Security tools operate on PostgreSQL catalog views (`pg_roles`, `pg_stat_ssl`, `pg_hba_file_rules`, `information_schema.columns`) and pure-JS logic. No user-created test tables are required. The existing seed includes tables with sensitive-looking columns (`test_secure_data.sensitive_data`, `test_users.email`, `test_employees.name`) which are used by `pg_security_sensitive_tables`.

> **Superuser note:** `pg_security_firewall_status` and `pg_security_firewall_rules` require superuser access to `pg_hba_file_rules`. The test server runs as `postgres` (superuser). If running against a non-superuser connection, these tools should return a structured error — not a raw MCP error.

**Checklist:**

1. `pg_security_audit()` → verify `{success: true, findings: [...], summary: {total, passed, warnings, critical}}` with severity levels
2. `pg_security_audit({limit: 2})` → verify findings array length ≤ 2
3. `pg_security_ssl_status()` → verify `{success: true, sslEnabled: boolean, sslConnections: [...], totalConnections: N}`
4. `pg_security_encryption_status()` → verify `{success: true, sslEnabled, passwordEncryption, pgcryptoAvailable, encryptionSettings, certificates}`
5. `pg_security_password_validate({password: "Str0ng!Pass#2026"})` → verify `strength >= 80`, `interpretation: "Very Strong"`, `meetsPolicy: true`
6. `pg_security_password_validate({password: "123456"})` → verify `strength < 20`, `checks.notCommon: false`
7. `pg_security_mask_data({value: "user@example.com", type: "email"})` → verify masked output preserves `@example.com` domain, local part is masked
8. `pg_security_mask_data({value: "4111111111111111", type: "credit_card"})` → verify first 4 and last 4 digits preserved, middle masked
9. `pg_security_mask_data({value: "555-12-3456", type: "ssn"})` → verify last 4 digits preserved, prefix masked (`***-**-3456`)
10. `pg_security_mask_data({value: "sensitive-data", type: "partial", keepFirst: 3, keepLast: 2})` → verify `sen*********ta`
11. `pg_security_user_privileges()` → verify `{success: true, users: [...], count: N}` listing non-system roles
12. `pg_security_user_privileges({user: "postgres"})` → verify single role with `isSuperuser: true`
13. `pg_security_user_privileges({summary: true})` → verify condensed output with `grantCount` and `roleCount` fields
14. `pg_security_sensitive_tables()` → verify returns tables matching default patterns (password, email, token, etc.)
15. `pg_security_sensitive_tables({schema: "public", patterns: ["email", "password"]})` → verify custom patterns used, `patternsUsed` matches input
16. `pg_security_firewall_status()` → verify `{success: true}` with HBA summary or structured permission error
17. `pg_security_firewall_rules()` → verify `{success: true, rules: [...], count: N}` or structured permission error

18. 🔴 `pg_security_password_validate({})` → `{success: false, error: "..."}` (missing required `password` param)
19. 🔴 `pg_security_mask_data({})` → `{success: false, error: "..."}` (missing `value` and `type`)
20. 🔴 `pg_security_mask_data({value: "test", type: "invalid_type"})` → `{success: false, error: "..."}` handler error for invalid masking type
21. 🔴 `pg_security_user_privileges({user: "nonexistent_role_xyz"})` → `{success: false, error: "Role 'nonexistent_role_xyz' does not exist."}` (P154)
22. 🔴 `pg_security_sensitive_tables({schema: "fake_schema_xyz"})` → `{success: false, error: "Schema 'fake_schema_xyz' does not exist..."}` (P154)
23. 🔴 `pg_security_firewall_rules({type: 999})` → `{success: false, error: "..."}` (Zod type mismatch — number instead of string)
