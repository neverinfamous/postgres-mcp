# postgres-mcp Tool Group Re-Testing: [pgcrypto]
...
9. `pg_pgcrypto_create_extension()` → verify happy path expected behavior
10. 🔴 `pg_pgcrypto_create_extension({})` → verify structured P154 error response or valid defaults
11. `pg_pgcrypto_hmac()` → verify happy path expected behavior
12. 🔴 `pg_pgcrypto_hmac({})` → verify structured P154 error response or valid defaults
13. `pg_pgcrypto_crypt()` → verify happy path expected behavior
14. 🔴 `pg_pgcrypto_crypt({})` → verify structured P154 error response or valid defaults
