# Advanced Stress Test — postgres-mcp — docstore Group

**ESSENTIAL INSTRUCTIONS**

- Execute **EVERY** numbered stress test below using code mode (`pg_execute_code`).
- Do not use scripts or terminal to replace planned tests, run any other test files, or do anything other than these tests. Ignore distractions in terminal from work being done in other thread.
...
- **Temporary collections**: Prefix with `stress_` (e.g., `stress_doc_test`)
...
```sql
-- List stress tables/collections
SELECT tablename FROM pg_tables
WHERE schemaname = 'public' AND tablename LIKE 'stress_%';

-- Drop stress collection
DROP TABLE IF EXISTS stress_doc_test;
```
...
## docstore Group Advanced Tests

### docstore Group Tools (9 + 1 code mode)

1. `pg_doc_list_collections`
2. `pg_doc_create_collection`
3. `pg_doc_drop_collection`
4. `pg_doc_collection_info`
5. `pg_doc_find`
6. `pg_doc_add`
7. `pg_doc_modify`
8. `pg_doc_remove`
9. `pg_doc_create_index`
10. `pg_execute_code` (auto-added)

### Category 1: Boundary Values & Empty States

Test tools against extreme parameters, zero-state inputs, and boundary sizing.

1. `pg_doc_find` → Supply `limit: 0` boundary edge case. Verify whether handler returns empty documents array or clamps to minimum natively.
2. `pg_doc_find` → Supply extreme `limit: 999999` on `test_documents` (5 rows). Verify handler does not crash and returns all 5 documents.
3. `pg_doc_add` → Supply empty documents array `documents: []`. Verify Zod or handler enforcement triggers structured error `{success: false}` — not raw MCP `-32602`.
4. `pg_doc_find` → Execute on `stress_doc_empty` collection (create it empty first). Verify clean empty array returned `{success: true, documents: [], count: 0}` — no null leakage.

### Category 2: State Pollution & Idempotency

Ensure tools execute safely when repeated identically multiple times.

5. `pg_doc_create_collection` → Create `stress_doc_dup`, then attempt to create same collection again. Verify second call returns structured duplicate error `{success: false}` — not a raw PostgreSQL `42P07` (duplicate table) MCP error.
6. `pg_doc_drop_collection` → Drop `stress_doc_dup`, then attempt to drop again. Verify second call returns structured error or success with IF EXISTS safety — not a raw error.
7. `pg_doc_list_collections` → Execute consecutively 3 times inside a single Code Mode script. Verify all 3 responses return identical collection counts — no state pollution.
8. `pg_doc_find` → Execute identical query on `test_documents` 3 times. Verify all 3 responses are structurally identical (same document count, same `_id` values).

### Category 3: Alias & Parameter Combinations

Test parametric fallback modes and configuration matrices.

9. `pg_doc_find` → Execute a filter operator matrix across all supported comparison operators: `$gt`, `$gte`, `$lt`, `$lte`, `$ne`, `$in`, `$nin` against `test_documents` using the `age` field. Verify each operator produces correct results:
   - `{age: {$gt: 30}}` → 2 docs (Charlie=35, Eve=32)
   - `{age: {$gte: 30}}` → 3 docs (Alice=30, Charlie=35, Eve=32)
   - `{age: {$lt: 28}}` → 1 doc (Bob=25)
   - `{age: {$lte: 28}}` → 2 docs (Bob=25, Diana=28)
   - `{age: {$ne: 30}}` → 4 docs (all except Alice)
   - `{age: {$in: [25, 35]}}` → 2 docs (Bob, Charlie)
   - `{age: {$nin: [25, 35]}}` → 3 docs (Alice, Diana, Eve)
10. `pg_doc_modify` → Progressive field mutations on a single document in `stress_doc_mutations`: set field `status: "draft"` → verify → set `status: "published"` → verify → unset `status` → verify field absent. Confirm each state transition is atomic and correct.

### Category 4: Error Message Quality

Ensure tools predictably return typed structured errors with quality messages.

11. `pg_doc_add` → Pass SQL injection attempt in document value: `documents: [{"name": "'; DROP TABLE test_products;--"}]` into `stress_doc_inject`. Verify safe JSONB handling — the document is stored literally as a string, no SQL injection occurs. Verify `test_products` still exists afterwards via `pg.count({table: "test_products"})`.
12. `pg_doc_find` → Pass filter with deeply nested path: `filter: {"address": {"city": {"$gt": "A"}}}`. Verify structured error or correct nested JSONB path traversal — not a raw PostgreSQL error.

### Category 5: Complex Flow Architectures

Verify that multi-tool pipelines compose correctly across the docstore tool surface.

13. Full Document Lifecycle Pipeline → Execute the following sequence in a single Code Mode script:
    - `pg.docstore.createCollection({collection: "stress_doc_pipeline"})` — create collection
    - `pg.docstore.add({collection: "stress_doc_pipeline", documents: [{name: "Item1", price: 10}, {name: "Item2", price: 20}, {name: "Item3", price: 30}, {name: "Item4", price: 40}, {name: "Item5", price: 50}, {name: "Item6", price: 60}, {name: "Item7", price: 70}, {name: "Item8", price: 80}, {name: "Item9", price: 90}, {name: "Item10", price: 100}]})` — add 10 docs
    - `pg.docstore.find({collection: "stress_doc_pipeline"})` → verify 10 docs
    - `pg.docstore.find({collection: "stress_doc_pipeline", filter: {price: {$gt: 50}}})` → verify 5 docs
    - `pg.docstore.modify({collection: "stress_doc_pipeline", filter: {price: {$lte: 30}}, set: {category: "budget"}})` → verify 3 modified
    - `pg.docstore.find({collection: "stress_doc_pipeline", filter: {category: "budget"}})` → verify 3 budget docs
    - `pg.docstore.remove({collection: "stress_doc_pipeline", filter: {category: "budget"}})` → verify 3 removed
    - `pg.docstore.find({collection: "stress_doc_pipeline"})` → verify 7 remaining
    - `pg.docstore.createIndex({collection: "stress_doc_pipeline", field: "price"})` — create index
    - `pg.docstore.collectionInfo({collection: "stress_doc_pipeline"})` → verify rowCount=7, index present
    - `pg.docstore.dropCollection({collection: "stress_doc_pipeline"})` — cleanup
    - Verify each step returns `{success: true}` and the full pipeline round-trips cleanly with zero residual state.

### Category 6: Large Payload & Truncation Verification

Ensure sweeping reads cap context window exposure.

14. `pg_doc_add` → Insert 100 documents with large nested JSONB (5 levels deep, arrays of objects) into `stress_doc_payload`. Then `pg_doc_find` with no filter. Monitor `metrics.tokenEstimate` strictly. Report if payload exceeds 3000 tokens. Verify `limit` parameter correctly caps output when applied. Drop `stress_doc_payload` at end.

### Final Cleanup

15. Verify no `stress_*` collections remain. Execute `pg.execute("SELECT tablename FROM pg_tables WHERE tablename LIKE 'stress_%' AND schemaname = 'public'")` and assert zero rows.
