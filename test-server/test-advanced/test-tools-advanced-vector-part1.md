# Advanced Stress Test — postgres-mcp — vector Group (Part 1)
...
**2.1 HNSW / IVFFLAT Idempotency** 3. Create an HNSW index on `stress_vector_table`. Call drop, then recreate identically. Ensure state logic bounds prevent deadlocks.
