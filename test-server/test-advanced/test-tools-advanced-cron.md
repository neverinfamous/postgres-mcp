...
9. `pg_cron_alter_job` → Create a job. Alter _only_ the `active` parameter. Verify other parameters remain intact.
10. `pg_cron_alter_job` → Alter _only_ the `schedule` parameter, leaving `active` and `command` independent.
11. `pg_cron_alter_job` → Alter _all_ parameters simultaneously: `{jobName: "stress_cron_multi", schedule: "0 0 * * *", command: "SELECT 2", active: true}`.
...
17. `pg_cron_job_run_details` → Query history for a job that was _just_ created (has 0 execution history logs). Verify it cleanly returns an empty array vs an object reference error.
