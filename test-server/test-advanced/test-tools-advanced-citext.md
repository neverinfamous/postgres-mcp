...
15. `pg_citext_create_extension` → Specify parameter `{schema: "stress_citext_schema"}`. Inspect if extension successfully registers out-of-core vs defaulting to public. _(Note: PostGres often globalizes this, just gauge tool behavior)_.
...
20. Validation: Immediately attempt to insert duplicate case-conflicting rows into the table (`admin@google.com` vs `ADMIN@google.com`). This _must_ fail on the unique constraint if CITEXT applied properly to the underlying index hierarchy.
