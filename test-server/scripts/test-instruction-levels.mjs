  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("exit", (code) => {
      if (code !== 0 && code !== null) {
        reject(
          new Error(
            `Server exited prematurely with code ${code} (ensure PostgreSQL is running on 5432)`,
          ),
        );
      }
    });
...
async function main() {
  ...
  console.log(
    `  Core includes Code Mode: ${coreHasCodeMode ? "✅" : "❌ MISSING"}`,
  );
  ...
  console.log(
    `  Full includes Code Mode: ${fullHasCodeMode ? "✅" : "❌ MISSING"}`,
  );
  ...
  const starterFilter = await testServer([
    "--tool-filter",
    "core,transactions,jsonb,schema,codemode",
  ]);
  console.log(
    `  explicit groups: ${starterFilter.charCount} chars (~${starterFilter.tokenEstimate} tokens)`,
  );
  ...
  console.log(
    `  explicit groups includes Code Mode: ${starterHasCodeMode ? "✅" : "❌ MISSING"}`,
  );
  ...
  console.log(
    `  essential: ${essential.charCount} chars (~${essential.tokenEstimate} tokens)`,
  );

  const standard = await testServer(["--instruction-level", "standard"]);
  console.log(
    `  standard:  ${standard.charCount} chars (~${standard.tokenEstimate} tokens)`,
  );

  const full = await testServer(["--instruction-level", "full"]);
  console.log(
    `  full:      ${full.charCount} chars (~${full.tokenEstimate} tokens)`,
  );

  // essential <= standard <= full
  const levelOrdering =
    essential.charCount <= standard.charCount &&
    standard.charCount <= full.charCount;
  console.log(
  ...
  console.log(
    `  essential excludes help group list: ${essentialHasHelp ? "❌ FOUND" : "✅"}`,
  );
  ...
  console.log(
    `  full includes Active Tools: ${fullHasActive ? "✅" : "❌ MISSING"}`,
  );
  ...
  console.log(
    `  standard excludes Active Tools: ${standardHasActive ? "❌ FOUND" : "✅"}`,
  );
