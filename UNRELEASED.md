...
- **Split Schema / Zod validation fixes** (consolidated across all tool groups):
  ...
  - **Cron**: Fixed Split Schema validation leaks, limit coercion, Zod boundary violations across schedule/cleanup/list tools, optimized pg_cron_job_run_details payload, added missing `success: true` to `pg_cron_list_jobs` and `pg_cron_job_run_details`
