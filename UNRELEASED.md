...
- Resolved structural validation issues preventing default parameter resolution (empty `{}` requests) by properly centralizing and separating `z.preprocess` logic from base schemas in `performance.ts` for all remaining performance tools (`pg_locks`, `pg_bloat_check`, `pg_diagnose_database_performance`).
...
- Centralized input schemas for `pg_seq_scan_tables` and `pg_index_recommendations` to resolve remaining Split Schema Pattern violations in the performance analysis tool group.
