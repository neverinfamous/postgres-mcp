...
- Restored `test_locations.location` geometry column and `idx_locations_geo` GiST index in `test-database.sql` after `DROP EXTENSION postgis CASCADE` (Test 9) dropped geometry-typed columns via CASCADE; added explicit column add-back and re-seeding step in the PostGIS Part 2 test teardown; added cautionary note to `test-tools-advanced-postgis-part2.md` regarding CASCADE column side-effects
- Verified `pg_distance` gracefully returns `COLUMN_NOT_FOUND` structured error (not raw MCP exception) when PostGIS is unavailable; error code clarification noted as a documentation-only finding (no code change required, handler behavior is correct)
- Corrected `postgis.md` server instructions: fixed stale default `limit: 50` to `limit: 10` for `pg_buffer` and `pg_geo_transform` (matching actual handler defaults); added `alreadyExists: true` note to `pg_postgis_create_extension`; added `warning` field note to `pg_point_in_polygon` for non-polygon column types; added structured error note to `pg_geo_index_optimize` for non-spatial tables
- Updated `performance.md` server instructions: added `locks({ showBlocked?, limit? })` and `statActivity({ includeIdle?, limit?, truncateQuery? })` entries to the AI-Optimized Payloads section (both tools have had bounded limit params since the certification cycle); added Extension Dependency note clarifying that `diagnoseDatabasePerformance` uses `pg_stat_activity` (no `pg_stat_statements` required), while `statStatements` and `queryPlanStats` return a structured `QUERY_ERROR` when the extension is absent
