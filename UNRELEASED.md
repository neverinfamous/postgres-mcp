...
- **Split Schema / Zod validation fixes** (consolidated across all tool groups):
  ...
  - **Cron**: Fixed Split Schema validation leaks, limit coercion, Zod boundary violations across schedule/cleanup/list tools, optimized pg_cron_job_run_details payload (limit 50→10)
