...
- `pg_citext_list_columns`: Default `limit: 50` (max `100`). Returns `{columns: [{table_schema, table_name, column_name, is_nullable, column_default}], count, totalCount, truncated}`. Optional `schema`, `limit` filters. Supports `compact` mode (default: `true`) to structurally omit empty arrays
