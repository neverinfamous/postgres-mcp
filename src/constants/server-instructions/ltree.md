...
- `pg_ltree_query`: Query hierarchical relationships. Supports `schema.table` format (auto-parsed). `mode`/`type`: 'ancestors', 'descendants' (default), 'exact'. Accepts `limit` (default 50). Returns `{results, count, path, mode, isPattern, truncated, totalCount}`. ⚠️ Validates column is ltree type—returns clear error for non-ltree columns
- `pg_ltree_match`: Match paths using lquery pattern syntax (`*`, `*{1,2}`, `*.label.*`). Supports `schema.table` format. `pattern`/`lquery`/`query` aliases. Accepts `limit` (default 50). Returns `{results, count, pattern, truncated, totalCount}`
