...
- `pg_kcache_query_stats`: `limit` param (default: 5, max: 10). Returns `truncated` + `totalCount` when limited. `orderBy`: 'total_time' (default), 'cpu_time', 'reads', 'writes'. `queryPreviewLength`: chars for query preview (default: 100, max: 500, 0 for full). `compact`: boolean (default: true, omits query_preview text). ⛔ 'calls' NOT valid for orderBy—use `minCalls` param
