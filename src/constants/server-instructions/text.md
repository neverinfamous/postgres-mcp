...
- **Aliases**: Parameters accept convenient aliases across tools: `tableName` → `table`, `col` → `column`, `filter` → `where`, `text` → `value`, `indexName` → `name`.
- `pg_text_search`: Supports both `column` (singular string) and `columns` (array). Either is valid—`column` auto-converts to array
- 📦 **AI-Optimized Payloads**: All row-returning text tools (`pg_text_search`, `pg_text_rank`, `pg_text_headline`, `pg_trigram_similarity`, `pg_fuzzy_match`, `pg_regexp_match`, `pg_like_search`) default to 100 results. Returns `truncated: true` + `hint` when capped. **Note:** `limit` cannot exceed 100, and `limit: 0` is strictly rejected to prevent Unbounded Payload errors. Use the `select` parameter to explicitly specify columns and avoid returning large `tsvector` footprints.
- `pg_fuzzy_match`: Levenshtein returns distance (lower=better). Soundex/metaphone return phonetic codes (exact match only). ⛔ Invalid `method` values return structured error with valid options.
