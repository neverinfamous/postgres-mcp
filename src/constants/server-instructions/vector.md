...
- `pg_vector_create_extension`: Quickly enable the `vector` extension in the target schema (defaults to `public`).
- `pg_vector_add_column`: Add a vector column with a specified number of dimensions. Accepts parameter aliases (`tableName`, `col`). Returns `{success, table, column, dimensions, ifNotExists}` (with `alreadyExists` when applicable).
...
- `pg_vector_dimension_reduce`: Direct mode returns `{success, reduced: [...], originalDimensions, targetDimensions}`. Table mode uses default limit 20 (max 100) and returns `{success, rows: [{id, original_dimensions, reduced}], processedCount, summarized}`. Default `summarize: true` in table mode returns compact `{preview, dimensions, truncated}` format. Use `summarize: false` for full vectors
