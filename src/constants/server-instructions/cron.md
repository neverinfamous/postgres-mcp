...
- `pg_cron_alter_job`: Modify existing job. Can change `schedule`, `command`, `database`, `username`, `active`. ⛔ Non-existent job returns structured error (uses jobId or jobName)
...
- `pg_cron_job_run_details`: View execution history. Default `limit: 10`. Optional `jobId`/`jobName`, `status` ('running'|'succeeded'|'failed') filters. Returns `truncated` + `totalCount` when limited. Returns `summary` with counts. Supports `compact` mode (default: `true`) to truncate long text fields like command and return_message
