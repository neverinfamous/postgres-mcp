 ...
 */
const HELP_GROUP_KEYS: readonly string[] = ["admin","backup","citext","cron","docstore","introspection","jsonb","kcache","ltree","migration","monitoring","partitioning","partman","performance","pgcrypto","postgis","roles","schema","security","stats","text","transactions","vector"]
...
20. **Vector Output Parsing**: Vectors read from DB via standard \`readQuery\` are strings—parse before passing back into tools: \`vec.replace(/^\\[|\\]$/g, '').split(',').map(Number)\`. (Using \`pg.vector\` tools natively handles this).
21. **Text Search Columns**: \`pg_text_search\`/\`pg_text_rank\` columns must be \`text\` type. Pre-built \`tsvector\` columns are NOT supported (wrapping with \`to_tsvector()\` fails). Use \`pg_read_query\` for native \`tsvector\` queries.
22. **Transactions Aborted State**: If a statement fails, the transaction enters an ABORTED state. Only \`ROLLBACK\` or \`ROLLBACK TO SAVEPOINT\` are accepted. \`commit\` will report the state, not silently rollback.
23. **JSONB Tool Strictness**: Array-only tools (\`pg_jsonb_insert\`) throw on objects. Object-only tools (\`diff\`, \`merge\`, \`keys\`) throw on arrays. Negative indices must be array format \`[-1]\`, not string \`"[-1]"\`. JSON paths require a \`$\` prefix (\`$.key\`).
24. **Stats Percentiles**: Empty array defaults to \`[0.25, 0.5, 0.75]\`. Mixing scales (0.1 and 50) returns a warning. Response keys use prefix (\`p25\`, \`p50\`).
25. **Migration Idempotency**: \`pg_migration_record\` only records metadata. You must use \`pg_migration_apply\` to actually execute the migration SQL.
