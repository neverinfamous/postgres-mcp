...
- \`pg_create_partition\`: Use \`parent\`/\`table\`/\`parentTable\` for parent. Use \`name\` or \`partitionName\` for partition. Use bounds aliases \`from\`/\`to\` (RANGE), \`values\` array (LIST), or \`modulus\`/\`remainder\` (HASH). Alternatively, \`forValues\` takes a raw SQL string. For DEFAULT partition, use \`isDefault: true\`. Supports \`schema.table\` format for \`parent\` (auto-parsed)
- \`pg_attach_partition\`/\`pg_detach_partition\`: Support \`schema.table\` format for \`parent\` (aliases: \`table\`, \`parentTable\`) and \`partition\` (aliases: \`partitionName\`, \`partitionTable\`) (auto-parsed). \`pg_attach_partition\` supports the same \`from\`/\`to\`, \`values\`, \`modulus\`/\`remainder\` bounds aliases as create. For DEFAULT partition, use \`isDefault: true\` or \`forValues: "DEFAULT"\`
