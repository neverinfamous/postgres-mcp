...
def register_resources(mcp: FastMCP, get_sql_driver_func: Callable[[], Awaitable[Union[SqlDriver, SafeSqlDriver]]]) -> None:
        ...
        try:
            ...
            capabilities: dict[str, Any] = {
                ...
                "key_capabilities": [
                    ...
                    "Real-time query performance via pg_stat_statements" if has_pg_stat else "pg_stat_statements not installed (CRITICAL for performance monitoring)",
    ...
    async def get_performance_metrics() -> str:  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            ...
            return json.dumps({
                "error": str(e),
                "recommendation": "Verify pg_stat_statements is installed and configured"
            }, indent=2)
            ...
            for ext_name in critical_extensions:
                if ext_name not in installed_names:
                    recommendations_ext.append({
                        ...
                        "reason": "Critical for performance monitoring" if ext_name == "pg_stat_statements" else "Enables risk-free index testing"
                    })
    ...
    async def get_connections_info() -> str:  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            ...
            if len(warnings_conn) == 0:
                warnings_conn.append({
                    "severity": "INFO",
                    "message": "Connection pool looks healthy"
                })
    ...
    async def get_replication_info() -> str:  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            ...
            return json.dumps({
                "error": str(e),
                "note": "Replication monitoring requires appropriate permissions"
            }, indent=2)
    ...
    async def get_vacuum_info() -> str:  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            ...
            vacuum_info: dict[str, Any] = {
                "vacuum_statistics": vacuum_stats,
                "transaction_id_wraparound": wraparound_info,
                "warnings": warnings_vac
            }
    ...
    logger.info("Registered 10 database resources: schema, capabilities, performance, health, extensions, indexes, connections, replication, vacuum, locks, statistics")
