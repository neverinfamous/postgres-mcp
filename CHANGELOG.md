...
- **`pg_kcache_top_io` raw MCP `-32602` error for invalid `type`/`ioType` enum values** — `pg_kcache_top_io({type: "invalid"})` and `pg_kcache_top_io({ioType: "invalid"})` now return `{success: false, error: "Invalid type/ioType value ..."}` instead of leaking raw MCP `-32602` Zod validation errors. Root cause: `KcacheTopIoSchemaBase` used `z.enum(["reads", "writes", "both"])` for both `type` and `ioType` fields, causing the MCP framework to reject invalid values before the handler's `try/catch` could intercept. Changed to `z.string().optional()` in the Base schema with handler-side enum validation matching the pattern used by `pg_kcache_query_stats` for `orderBy`
