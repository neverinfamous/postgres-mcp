...
- **Schema tools wrong-type numeric params leak raw MCP `-32602` errors** — `pg_create_sequence({start: "abc"})`, `pg_list_sequences({limit: "abc"})`, `pg_list_views({limit: "abc", truncateDefinition: "abc"})`, and `pg_list_functions({limit: "abc"})` now silently fall back to their default values instead of crashing with raw MCP `-32602` Zod validation errors. Root cause: `z.coerce.number()` in `inputSchema` Base schemas converts `"abc"` to `NaN`, which fails Zod validation at the MCP framework level before the handler's `try/catch`. Fix: changed to `z.any().optional()` in 9 Base schema params across `schema-mgmt.ts` (`CreateSequenceSchemaBase`: 5 params — `start`, `increment`, `minValue`, `maxValue`, `cache`; `ListFunctionsSchemaBase`: 1 param — `limit`), `views.ts` (`pg_list_views`: 2 params — `truncateDefinition`, `limit`), and `objects.ts` (`pg_list_sequences`: 1 param — `limit`). Added `safeCoerceNumber` helper in `schema-mgmt.ts` for `CreateSequenceSchema` transform, and `Number()` + `Number.isFinite()` coercion in all 4 list tool handlers
...
- **4 schema tools raw MCP `-32602` error for empty params** — `pg_create_schema({})`, `pg_drop_schema({})`, `pg_drop_view({})`, and `pg_drop_sequence({})` now return `{success: false, error: "name is required"}` instead of raw MCP `-32602` Zod validation errors. Root cause: `CreateSchemaSchema`, `DropSchemaSchema`, `DropViewSchemaBase`, and `DropSequenceSchemaBase` had `name: z.string()` (required) in their `inputSchema`, causing the MCP framework to reject empty params before the handler's `try/catch`. Applied Split Schema pattern: created `CreateSchemaSchemaBase` and `DropSchemaSchemaBase` with `name: z.string().optional()`, made `name` optional in `DropViewSchemaBase` and `DropSequenceSchemaBase`, and added `.refine()` to all 4 full schemas ensuring `name` is non-empty. Handlers use `?? ""` fallback after parse for TypeScript type narrowing. Added 4 unit tests
