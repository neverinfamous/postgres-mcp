...
- **`pg_cron_job_run_details` raw MCP error for invalid status** — `pg_cron_job_run_details({ status: "invalid" })` now returns `{runs: [], count: 0, error: "Invalid status \"invalid\". Valid statuses: running, succeeded, failed"}` instead of a raw MCP Zod validation error (`-32602`). Changed `status` from `z.enum()` to `z.string()` with handler-level validation. Applied Split Schema pattern: `CronJobRunDetailsSchemaBase` for MCP visibility. Handler wrapped in try-catch with `ZodError` interception and `formatPostgresError` fallback. Added 1 unit test
