...
- **JSONB tools wrong-type optional numeric params leak raw MCP error** — Passing a string (e.g., `"abc"`) for optional numeric parameters (`sampleSize`, `topKeysLimit`, `limit`) on 7 JSONB tools now silently coerces to the default value instead of returning a raw MCP `-32602` Zod validation error. Root cause: `z.number().optional()` in `SchemaBase` definitions (`basic.ts`, `advanced.ts`) caused the MCP framework to reject wrong-type inputs before the handler's `try/catch`. Changed to `z.any().optional()` in all 8 `SchemaBase` params across `basic.ts` (4 `limit`) and `advanced.ts` (3 `sampleSize` + 1 `topKeysLimit`). Added `Number()` coercion with `isNaN` → default fallback in `analytics.ts` and `read.ts` handlers. Affected tools: `pg_jsonb_stats`, `pg_jsonb_index_suggest`, `pg_jsonb_security_scan`, `pg_jsonb_contains`, `pg_jsonb_path_query`, `pg_jsonb_extract`, `pg_jsonb_agg`. Added 2 unit tests
