...
- **`pg_jsonb_strip_nulls` inconsistent error prefix** — `pg_jsonb_strip_nulls` validation errors now use `Validation error: ...` prefix instead of the non-standard `pg_jsonb_strip_nulls validation error: ...`, consistent with all other tool handlers
