...
- **Transaction tools raw MCP exceptions** — `pg_transaction_begin`, `pg_transaction_commit`, `pg_transaction_rollback`, `pg_transaction_savepoint`, `pg_transaction_release`, and `pg_transaction_rollback_to` now route errors through `parsePostgresError()` for structured messages instead of raw MCP exceptions. Covers invalid transaction IDs (`TransactionError`), nonexistent savepoints (`3B001`), and aborted transaction state (`25P02`). `pg_transaction_execute` already had correct error handling. Added idempotency guard in `parsePostgresError` to prevent double-processing when adapter-level error handling has already structured the error (errors with `cause` but no PG `code` are re-thrown unchanged). Added 3 unit tests
