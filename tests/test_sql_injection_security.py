...
from postgres_mcp.sql import SqlDriver, SafeSqlDriver, DbConnPool
from postgres_mcp.server import AccessMode, get_sql_driver, current_access_mode
...
class PostgresSQLInjectionTester:
    ...
    def get_injection_test_cases(self) -> List[InjectionTest]:
        ...
        tests = [
            ...
            InjectionTest(
                ...
                payload="1' AND (SELECT CASE WHEN (SELECT COUNT(*) FROM test_users WHERE is_admin = TRUE) > 0 THEN pg_sleep(3) ELSE pg_sleep(0) END) IS NOT NULL--",
                expected_vulnerable=True,
                security_level=SecurityLevel.HIGH,
    ...
    def generate_security_report(self, results: Dict[str, List[TestResult]]) -> Dict[str, Any]:
        ...
        for mode, mode_results in results.items():
            ...
            critical_vulns = sum(1 for r in mode_results 
                               if r.vulnerable and r.test.security_level == SecurityLevel.CRITICAL)
            high_vulns = sum(1 for r in mode_results 
                           if r.vulnerable and r.test.security_level == SecurityLevel.HIGH)
            medium_vulns = sum(1 for r in mode_results 
                             if r.vulnerable and r.test.security_level == SecurityLevel.MEDIUM)
