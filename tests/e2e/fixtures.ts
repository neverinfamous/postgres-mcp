...
export const test = baseTest.extend<{}, WorkerFixtures>({
  workerServer: [
    async ({}, use, workerInfo) => {
      ...
      try {
        console.log(
          `[Worker ${workerIndex}] Provisioning database ${dbName}...`,
        );

        // Terminate active connections to allow drop
        try {
          execSync(
            `docker exec -e PGPASSWORD=${password} postgres-server psql -U postgres -d postgres -t -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${dbName}';"`,
            { stdio: "ignore" },
          );
        } catch {}

        execSync(
          `docker exec -e PGPASSWORD=${password} postgres-server psql -U postgres -d postgres -c "DROP DATABASE IF EXISTS ${dbName};"`,
          { stdio: "ignore" },
        );

        // Create fresh clone template from postgres database
        execSync(
          `docker exec -e PGPASSWORD=${password} postgres-server psql -U postgres -d postgres -c "CREATE DATABASE ${dbName};"`,
          { stdio: "ignore" },
        );

        // Seed it from the container's /tmp/ artifacts
        execSync(
          `docker exec -e PGPASSWORD=${password} postgres-server psql -U postgres -d ${dbName} -f /tmp/test-database.sql`,
          { stdio: "ignore" },
        );
        execSync(
          `docker exec -e PGPASSWORD=${password} postgres-server psql -U postgres -d ${dbName} -f /tmp/test-resources.sql`,
          { stdio: "ignore" },
        );
      } catch (err: any) {
        console.error(
          `[Worker ${workerIndex}] Failed to provision database ${dbName}:`,
          err.message,
        );
        throw err;
      }
      ...
      console.log(
        `[Worker ${workerIndex}] Starting isolated MCP server on port ${port}...`,
      );
      ...
      // 3. Teardown
      console.log(
        `[Worker ${workerIndex}] Tearing down MCP server on port ${port}...`,
      );
      stopServer(port);
