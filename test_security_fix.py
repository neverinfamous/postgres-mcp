...
async def test_security_fix():
    ...
    try:
        ...
        result = await execute_sql(sql="SELECT %s as user_input, 'Parameter binding blocked injection!' as status", params=[malicious_input])
        ...
        result = await execute_sql(
            sql="SELECT %s as name, %s as age, %s as is_admin, %s as dangerous_input", params=["Alice", 25, False, "'; DROP TABLE users; --"]
        )
